...
//+,liuyongliang.wt,add,2023.4.30, add hard info with ili7807s-tianma
...
//-,liuyongliang.wt,add,2023.4.30, add hard info with ili7807s-tianma
int focal_lcd_need_reset;//wangcong.wt,add,2023.7.31, add esd
EXPORT_SYMBOL(focal_lcd_need_reset);
...
#endif
//-,wangcong.wt,add,2023.8.28, add lcm_get_serialnum
//+P86801AA1,peiyuexiang.wt,modify,2023/09/04,add lcm_name identify
char Lcm_name_tp[255];
EXPORT_SYMBOL(Lcm_name_tp);
//-P86801AA1,peiyuexiang.wt,modify,2023/09/04,add lcm_name identify
int esd_interval_time = 0;
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
			if (config->return_buf[i] !=
				...
				//+P86801AA1,liuyongliang.wt,add,2023.07.07, add log
...
static int dsi_display_tpfw_status_read(struct dsi_display *display)
{
	int rc = 1;
	pr_info("WT_LCD,  focal_lcd_need_reset=%d\n", focal_lcd_need_reset);
	if (focal_lcd_need_reset) {
		focal_lcd_need_reset = 0;
		return -EINVAL;
	}
	return rc;
}
...
static int dsi_display_status_reg_read(struct dsi_display *display)
{
	...
	pr_info("WT_LCD,dsi_display_validate_status\n");
					...
					bool te_check_override)
{
	...
	if (status_mode == ESD_MODE_REG_READ) {
		if ((!strcmp("ft8203_wt_dsi_vdo_90hz_focaltech", panel->name)) || (!strcmp("focaltech_ft8203_wt_dsi_vdo_90hz_apf_boe", panel->name)) || (!strcmp("dsbj_ft8203_wt_dsi_vdo_90hz_inx", panel->name))) {
			rc = dsi_display_tpfw_status_read(dsi_display);
			pr_info("WT_LCD,ft8203 panel rc=%d\n", rc);
		} else {
			rc = dsi_display_status_reg_read(dsi_display);
			pr_info("WT_LCD,else panel  rc=%d\n", rc);
		}
...
//+,liuyongliang.wt,add,2023.4.30, add hard info with ili7807s-tianma
	...
	if(!strcmp("qcom,mdss_dsi_ft8203_video_display",display->name))
	{
	...
	else if(!strcmp("qcom,mdss_dsi_ft8203_apf_boe_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_hx83102j_starry_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_hx83102j_starry_agc_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_hx83102j_lide_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	...
	else if(!strcmp("qcom,mdss_dsi_ft8203_dsbj_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_icnl9951r_hjr_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_icnl9951r_txd_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_icnl9951r_xinxian_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	}
	else if(!strcmp("qcom,mdss_dsi_icnl9951r_xinxian_agc_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
	...
	else if(!strcmp("qcom,mdss_dsi_virtual_video_display",display->name))
	{
		strlcpy(Lcm_name, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), HARDWARE_MAX_ITEM_LONGTH);
		strlcpy(Lcm_name_tp, utils->get_property(utils->data,"qcom,mdss-dsi-panel-name", NULL), 255);
		DSI_INFO("Successfully bind display panel '%s'\n", display->name);
		...
		DSI_INFO("[display]: bind display panel fail\n");
//-,liuyongliang.wt,add,2023.4.30, add hard info with ili7807s-tianma
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	//+,wangcong.wt,add,2023.8.28, add lcm_get_serialnum
	#ifdef CONFIG_QGKI_BUILD
	pr_info("lcm_get_serialnum display->display_type: %s\n", display->display_type);
	if(!strcmp(display->display_type, "primary")) {
